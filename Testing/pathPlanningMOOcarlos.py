...
class EvolutionaryPathPlanner:
    ...
    def breeding(self, ind1, ind2):
        ...
        # print("Breeding Function:")
        # print("first indivdual control points ([x y]):\n")
        # print (ind1[0])
        # print("\n")
        # print("first indivdual weights:\n")
        # print (ind1[1])
        # print("\n")
        # print("second indivdual control points ([x y]):\n")
        # print (ind2[0])
        # print("\n")
        # print("second indivdual weights:\n")
        # print(ind2[1])
        # print("\n")
        ...
        # Expanded version of the previous line to compute child_cp 
        def breeding(self, ind1, ind2):
            ...
            avg_sigmaxy = (
                [(x + y) / 2 for x, y in zip(sigmaxy1[0], sigmaxy2[0])],
                [(x + y) / 2 for x, y in zip(sigmaxy1[1], sigmaxy2[1])]
            )



            # Expanded versipn of the previous line to compute child_weights
            # child_weights = []

            # for weight1, weight2 in zip(w1, w2):
            #     average_weight = (weight1 + weight2) / 2
            #     child_weights.append(average_weight)

            # print("Result of breeding:\n")
            # print("Child Control Points ([x y]):\n")
            # print(child_cp)
            # print("\n")
            # print("Child Weights (obtained from the child_weight array)")
            # print(child_weights)
            ...
            # Replace sigmaxy values in individuals
            ind1 = (cp1, w1, avg_sigmaxy, sigmaw1)
            ind2 = (cp2, w2, avg_sigmaxy, sigmaw2)

            # Construct and return the child individual
            return ind1, ind2, avg_sigmaxy

    def mutation(self, individual, n_control_points):
        # n_control_points = len(individual)
        D = 3 * n_control_points - 4  # Calculate D based on the number of control points
        l = 1; #hyperparameter to be adjusted 
        tau0 = l / np.sqrt(2 * D)
        tau = l / np.sqrt(2 * np.sqrt(D))
        xi = np.random.normal()
        ...
        D = 3 * n_control_points - 4
        l = 1.0  # hyperparameter to be adjusted
        tau0 = l / np.sqrt(2 * D)
        tau = l / np.sqrt(2 * np.sqrt(D))
        xi = np.random.normal()

        # Apply mutation to control points
        mutated_ctrlpts = ctrlpts + np.exp(tau * xi) * sigmaxy

        # Apply mutation to weights
        mutated_weights = weights + np.exp(tau * xi) * sigmaw

        # Create the mutated individual
        mutated_individual = (mutated_ctrlpts, mutated_weights, sigmaxy, sigmaw)

        

        # print("Mutated control points: \n")
        # print(mutated_cp)
        # print("\n")

        # Mutate weights
        mutated_weights = [w + np.exp(tau0 * np.random.normal() + tau * np.random.normal()) * np.random.normal(scale=0.5) for w in weights]
        # Expanded form of the previous line
        # mutated_weights = []

        # for w in weights:
        #     random_term_1 = np.exp(tau0 * np.random.normal())
        #     random_term_2 = np.exp(tau * np.random.normal())
        #     random_term_3 = np.random.normal(scale=0.5)
            
        #     mutated_weight = w + random_term_1 * random_term_2 * random_term_3
    ...
    def run_evolution(self, initial_population, max_generations, no_improve_limit):
        ...
        for gen in range(max_generations):
            ...
            # print("Offspring before the mating loop\n")
            # print(offspring[0],"\n")
            # print(offspring[1],"\n")
            # print(offspring[2],"\n")
            ...
            for child1, child2 in zip(offspring[::2], offspring[1::2]):
                if random.random() < self.crossover_prob:
                    result = self.toolbox.mate(child1, child2)
                    # result should equal the return value of the breeding function 
                    # breeding returns ind1, ind2, avg_sigmaxy

            # print("Offspring afetr the mating loop")
            # print(offspring[0],"\n")
            # print(offspring[1],"\n")
            # print(offspring[2],"\n")
...
def main():
    ...
    mutation_prob = 1.0   # Example mutation probability
    epp = EvolutionaryPathPlanner(env, crossover_prob, mutation_prob)

    # Assuming combined_nurbs_paths is a list of NURBS curves
